...
void addItem(Player *p, Item it) {
    if (p->invCount >= MAX_INV) {
        printf("Inventory full. Cannot add %s.\n", it.name);
        return;
    }
    p->inventory[p->invCount++] = it;
    printf("Added %s to inventory.\n", it.name);
}

void useItem(Player *p) {
    if (p->invCount == 0) { printf("No items.\n"); pressEnterToContinue(); return; }
    printf("Choose item slot to use (1-%d) or 0 to cancel: ", p->invCount);
    int sel = getInt();
    if (sel <= 0 || sel > p->invCount) { printf("Canceled.\n"); pressEnterToContinue(); return; }
    Item it = p->inventory[sel-1];
    if (it.healAmount > 0) {
        p->hp += it.healAmount;
        if (p->hp > p->maxHp) p->hp = p->maxHp;
        printf("You used %s and healed %d HP. Now HP: %d/%d\n", it.name, it.healAmount, p->hp, p->maxHp);
    } else if (it.attackBonus > 0) {
        p->attack += it.attackBonus;
        printf("You used %s and gained +%d attack (permanently).\n", it.name, it.attackBonus);
    } else {
        printf("Nothing happened.\n");
    }
    // remove item by shifting
    for (int i = sel-1; i < p->invCount-1; ++i)
...
void shop(Player *p) {
    ...
    for (int i = 0; i < shopItemsCount; ++i) {
        printf("%d. %s (Price: %d", i+1, shopItems[i].name, shopItems[i].price);
        if (shopItems[i].healAmount>0) printf(", heal:%d", shopItems[i].healAmount);
        if (shopItems[i].attackBonus>0) printf(", atk+:%d", shopItems[i].attackBonus);
        printf(")\n");
    }
    printf("You have %d gold.\n", p->gold);
    printf("Choose item to buy (1-%d) or 0 to exit: ", shopItemsCount);
    int sel = getInt();
    if (sel <= 0 || sel > shopItemsCount) {
        printf("Exiting shop.\n");
        pressEnterToContinue();
        return;
    }
    Item it = shopItems[sel-1];
    if (p->gold >= it.price) {
        p->gold -= it.price;
        addItem(p, it);
        printf("You bought %s for %d gold.\n", it.name, it.price);
    } else {
...
void saveGame(const Player *p) {
    ...
    if (!f) { printf("Failed to open save file.\n"); return; }
    if (fwrite(p, sizeof(Player), 1, f) != 1) {
...
void sortInventoryByName(Player *p) {
    for (int i = 0; i < p->invCount-1; ++i) {
        for (int j = 0; j < p->invCount-1-i; ++j) {
            if (strcmp(p->inventory[j].name, p->inventory[j+1].name) > 0) {
                Item tmp = p->inventory[j];
...
void sortInventoryByPrice(Player *p) {
    for (int i = 0; i < p->invCount-1; ++i) {
        for (int j = 0; j < p->invCount-1-i; ++j) {
            if (p->inventory[j].price > p->inventory[j+1].price) {
                Item tmp = p->inventory[j];
                p->inventory[j] = p->inventory[j+1];
                p->inventory[j+1] = tmp;
            }
...
int searchItemByName(const Player *p, const char *name) {
    for (int i = 0; i < p->invCount; ++i) {
...
/*-------main-------*/ //AKHIRNYA SELESAIAAAAAAAA
int main() {
    srand((unsigned int)time(NULL));
    Player player;
    mainMenu(&player);
    printf("Thank you for playing this crappy game. semoga crashout.\n");
    return 0;
}